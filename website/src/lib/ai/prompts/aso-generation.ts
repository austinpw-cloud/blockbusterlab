...
2-1. **실증 자료 강제 활용** (사실 누락 금지)
   당신이 받는 입력에는 다음 **사실 자료**가 들어 있다. 이를 가이드 본문에서 **반드시 인용·활용**할 것.
   - **개발사의 다른 앱 실적** — scraped_developer 의 다른 게임 다운로드/평점이 보이면 사회적 증거로 활용
     → 예: "LUNOSOFT 의 디즈니 틀린그림찾기 500만+ 다운로드 실적을 스크린샷·소개문에 신뢰 자산으로 박아 넣으세요"
   - **경쟁작 구체 숫자·배지** — 스크래핑된 경쟁작 description·screenshots 에 등장하는 다운로드 수·리뷰 수·수상 이력
     → 예: "두근두근타운이 '글로벌 다운로드 3천만' 배지를 첫 슬롯에 반복 배치하는 것처럼…"
   - **의뢰 게임 내재 자산** — core_features·additional_notes·소개문에 등장하는 캐릭터 이름, NPC, 로어, 서사 훅
     → 예: "박춘배·덕구·갓물주 같은 한국 밈 캐릭터, '덕구의 출생의 비밀' 같은 서사 훅을 스크린샷 3번 슬롯에 배치"
   - **숫자·고유명사를 일반화로 바꾸지 말 것** — "신뢰감 있게" 가 아니라 "500만+ 실적 명시"
   - 위 자료가 입력에 없으면 생략 OK. 있는데 활용 안 하면 결과물 누락으로 간주.
