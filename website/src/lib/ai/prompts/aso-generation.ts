...
- 입력으로 주어진 **ASO bible·optimization guide 문서의 원칙**, **Reference Library(장르·국가별 Top 게임 벤치마킹 자산)**, **실시간 수집된 경쟁작 데이터(리뷰·수익모델 포함)** 를 종합해 판단합니다.
...
export function buildAsoGenerationPrompt(input: AsoGenerationInput): string {
  ...
  const reviewsBlock =
    input.scraped_reviews && input.scraped_reviews.length > 0
      ? `
### 실제 유저 리뷰 샘플 (평점별 혼합)
${input.scraped_reviews
  .slice(0, 20)
  .map(
    (r) =>
      `- [★${r.rating}] ${r.text.replace(/\s+/g, " ").slice(0, 200)}`
  )
  .join("\n")}
`
      : "";
...
각 경쟁작의 "실제 현재" 스토어 문구·리뷰·수익모델을 제공합니다.
당신의 추천에서 최소 3개 경쟁작을 실명으로 인용하고,
competitive_insight.competitors_analyzed 에 각 신규 축을 반드시 채우세요.
    ...
    const reviewSnippet =
      c.reviews && c.reviews.length > 0
        ? `\n**유저 리뷰 샘플 (평점별 혼합):**\n${c.reviews
            ...
            .map(
              (r) =>
                `- [★${r.rating}] ${r.text.replace(/\s+/g, " ").slice(0, 180)}`
            )
            .join("\n")}`
        : "";
...
- 개발사: ${c.developer}
- 장르 분류: ${c.genre}
- 평점: ${c.rating.toFixed(2)}
- 다운로드: ${c.installs}
${c.short_description ? `- 짧은 설명: ${c.short_description}` : ""}

**스토어 소개문 (첫 800자):**
${c.description_first_800}${reviewSnippet}${monSnippet}
`;
  })
  .join("\n---\n")}
`
...
${scrapedSection}${notesSection}${competitorSection}

## 장르 메타 (참고용 요약 — 일반 가이드, 절대 규칙 아님)
- 아이콘 스타일 관습: ${benchmark.icon_style}
- 스크린샷 스타일 관습: ${benchmark.screenshot_style}
- 장르 평균 평점: ${benchmark.avg_rating}
- 장르 핵심 인사이트:
${benchmark.key_insights.map((i) => `  - ${i}`).join("\n")}

⚠️ 위 요약은 과거 한국 시장 Top 중심 벤치마크에서 추출됨. 타겟 시장이 일본·미국·중국 등이면
  시스템 프롬프트의 "타겟 시장별 특성"을 우선 적용하고, 제공된 경쟁작(타겟 시장 실시간 수집)을
  더 신뢰할 것.
    ...
    "competitors_analyzed": [
      {
        ...
        // 신규 심화 축 (모두 채우세요 — '왜 Top인가'의 근거 체계)
        "why_they_top": "설치수·평점·리뷰를 근거로 '왜 이 게임이 정상인가' 1-2문장 (정량+정성 통합)",
        "core_hook": "메인 유인 요소 — 성취·수집·과시·힐링·경쟁·탐험 등 어떤 욕구를 정확히 건드리는가",
        "emotional_appeal": "유저가 이 게임에서 느끼는 주 감정과 그걸 일으키는 시각·문구 요소",
        "community_signals": {
          "rating_overall": "높/중/낮 + 평점 숫자",
          "praise_themes": ["리뷰에서 자주 나오는 칭찬 주제 2-4개"],
          "complaint_themes": ["자주 나오는 불만 주제 2-4개 (없으면 빈 배열)"]
        },
        "monetization_model": "IAP·광고·프리미엄·구독 등 + ASO 메시징에 어떻게 반영되는가",
        "retention_promise": "계속 플레이하게 만드는 신호 (레벨·시즌·컬렉션·소셜 등)",
        ...
        "direct_confrontation_risk": "이 경쟁작에 정면 승부하면 지는 이유 (우리 게임 관점)"
      }
      // 3-5개
    ],
    "white_space": [
